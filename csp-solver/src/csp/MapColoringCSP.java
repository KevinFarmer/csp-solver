...
public class MapColoringCSP extends ConstraintSatisfactionProblem {
	...
	public MapColoringCSP(ArrayList<String> countries, ArrayList<String> colors, boolean mrv, boolean lcv, boolean mac3) {
	...
	protected void buildConstraints() {
		...
		for (int i = 0; i < numVar; i++) {
			for (int j = i+1; j < numVar; j++) {
				...
				Constraint binaryConst = new Constraint(pair, validValues);
				constraints.put(pair, binaryConst);
			}
		}
	...
	private boolean isValidPair(int[] vals) {
		if (vals[0] != vals[1])
			return true;
		return false;
	}
